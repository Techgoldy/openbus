...
public class PostfixParser extends BaseFunction {
	...
	if (matcher.find()) {
	    ...
	    colector.emit(new Values(fechaFormato(matcher.group("EVENTTIMESTAMP")), matcher.group("SMTPDID"), matcher.group("MSGID"), matcher.group("CLEANUPID"), matcher
		    .group("QMGRID"), matcher.group("SMTPID"), matcher.group("ERRORID"), matcher.group("CLIENTE"), matcher.group("CLIENTEIP"), matcher.group("ACCION"), matcher
		    .group("SERVER"), matcher.group("SERVERIP"), matcher.group("MESSAGEID"), matcher.group("FROM"), matcher.group("SIZE"), matcher.group("NRCPT"), matcher
		    .group("TO"), matcher.group("TOSERVERNAME"), matcher.group("TOSERVERIP"), matcher.group("TOSERVERPORT"), matcher.group("DELAY"), matcher.group("DSN"), matcher
		    .group("STATUS"), matcher.group("STATUSDESC"), amavisId // ,matcher.group("AMAVISID")
	    ));
	}
    }
	...
	if (entrada != null && entrada.length() == 15) {
	    ...
	    // c.set(c.get(Calendar.YEAR), mes,
	    // Integer.parseInt(entrada.substring(4,6)),
	    // Integer.parseInt(entrada.substring(7,9)),
	    // Integer.parseInt(entrada.substring(10,12)),
	    // Integer.parseInt(entrada.substring(13,15)));
	}
