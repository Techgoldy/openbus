...
public class OpenbusHdfsState implements State {
	...
	protected TimeStampRotationPolicy rotationPolicy;
	protected OpenbusFileNameFormat fileNameFormat;
	...
	long maxPeriodoMillis;
	long inicioPeriodoMillis = 0;
	...
	public HdfsFileOptions withFileNameFormat(OpenbusFileNameFormat fileNameFormat) {
	...
	public HdfsFileOptions withRotationPolicy(TimeStampRotationPolicy rotationPolicy) {
	...
	public HdfsFileOptions addSyncMillisPeriod(long millis) {
	    if (millis > 0) {
		this.maxPeriodoMillis = millis;
	    }
	    return this;
	}
	...
	Path createOutputFile() throws IOException {
	    Path path = new Path(this.fileNameFormat.getPath(), this.fileNameFormat.getName(this.rotation, System.currentTimeMillis(), rotationPolicy.getBaseTimestamp()));
	...
	public void execute(List<TridentTuple> tuples) throws IOException {
		...
		// Cálculo del tamaño de la tupla
		byte[] bytes = this.format.format(tuple);
		this.offset += bytes.length;

		// eventTimeStamp de la tupla
		cal = Calendar.getInstance();
		// inicialización para la primera tupla
		if (inicioPeriodoMillis == 0) {
		    inicioPeriodoMillis = cal.getTimeInMillis();
		}

		if (this.rotationPolicy.markTimePeriod(tuple)) {
		    // Recibimos un registro con TimeStamp fuera del rango
		    // actual
		    // Rotamos el TimeStamp
		    this.rotationPolicy.resetTimePeriod();
		    // Reseteamos el tamaño, ya que cambiamos de fichero
		    this.offset = bytes.length;
		    this.rotationPolicy.reset();
		    // rotar el fichero
		    rotateOutputFile();
		    // reseteamos el tiempo base de sincronización
		    inicioPeriodoMillis = cal.getTimeInMillis();
		}
		else {
		...
		// Escritura en el fichero actual
		out.write(bytes);

		// Control de sincronización de escritura a HDFS
		if (this.maxPeriodoMillis < (cal.getTimeInMillis() - inicioPeriodoMillis)) {

		    this.out.hsync();
		    ((HdfsDataOutputStream) out).hsync(EnumSet.of(SyncFlag.UPDATE_LENGTH));
		    inicioPeriodoMillis = Calendar.getInstance().getTimeInMillis();
		}
	...
	public SequenceFileOptions withFileNameFormat(OpenbusFileNameFormat fileNameFormat) {
	    this.fileNameFormat = fileNameFormat;
	    return this;
	}
	...
	public SequenceFileOptions withRotationPolicy(TimeStampRotationPolicy rotationPolicy) {
	    this.rotationPolicy = rotationPolicy;
	    return this;
	}
	...
	Path createOutputFile() throws IOException {
	    ...
	    this.writer = SequenceFile.createWriter(this.hdfsConfig, SequenceFile.Writer.file(p), SequenceFile.Writer.keyClass(this.format.keyClass()),
    ...
    public static final Logger LOG = LoggerFactory.getLogger(OpenbusHdfsState.class);
    private Options options;

    OpenbusHdfsState(Options options) {
